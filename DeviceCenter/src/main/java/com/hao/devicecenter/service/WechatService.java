...
import com.hao.devicecenter.mapper.WechatUserInfoMapper;
import com.hao.devicecenter.config.WechatConfig;
...
public class WechatService {
    ...
    @Autowired
    protected WechatUserInfoMapper wechatUserInfoMapper;
...
//        checkStateLegal(state, request);
    ...
    private WechatUserInfo saveWechatUserInfo(String app, WechatAccess wechatAccess) {
        ...
        if (StringUtils.isNoneBlank(unionid)) {
            ...
            Set<WechatUserInfo> set = wechatUserInfoMapper.findByUniond(unionid);
            if (!CollectionUtils.isEmpty(set)) {
                ...
                if (userInfo != null) {
                    ...
                    UserCredential userCredential = UserCredential.builder().username(wechatUserInfo.getOpenid()).type(CredentialType.WECHAT_OPENID.name()).userId(userInfo.getUserId()).build();
                    userCredential.insert();
                }
            }
        }
